...
TListaCom TAVLCom::Inorden() const
{
	TListaCom recorrido;
...
void TAVLCom::InordenAux(TListaCom &l) const
{
	if(raiz != NULL)
	{
		raiz->iz.InordenAux(l);
		if(l.EsVacia())
			l.InsCabeza(raiz->item);
		else
			l.InsertarD(raiz->item, l.Ultima());	
...
TListaCom TAVLCom::Preorden() const
{
	TListaCom recorrido;
...
void TAVLCom::PreordenAux(TListaCom &l) const
{
	if(raiz != NULL)
	{
		if(l.EsVacia())
...
TListaCom TAVLCom::Postorden() const
{
	TListaCom recorrido;
...
void TAVLCom::PostordenAux(TListaCom &l) const
{
	if(raiz != NULL)
	{
...
TListaCom TAVLCom::Niveles() const
{
	TListaCom recorrido;
	TColaAVLNodo colaNodos;
	TAVLNodo *evaluado;

	if(raiz) colaNodos.Encolar(raiz);
	while(!colaNodos.EsVacia())
	{
		evaluado = (TAVLNodo *) colaNodos.Desencolar();
		if(recorrido.EsVacia())
			recorrido.InsCabeza(evaluado->item);
		else
			recorrido.InsertarD(evaluado->item, recorrido.Ultima());
		if(evaluado->iz.raiz)
			colaNodos.Encolar(evaluado->iz.raiz);
		if(evaluado->de.raiz)
			colaNodos.Encolar(evaluado->de.raiz);
	}
